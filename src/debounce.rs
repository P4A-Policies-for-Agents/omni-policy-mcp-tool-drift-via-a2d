...
//! (log line today, A²D `/mcp/evidence` POST once the HTTP client is
//! wired). This module keeps emission to at most 1 row per
...
//! - Time is passed in as `now_secs`; callers use
//!   [`now_epoch_secs`] which reads `SystemTime` (available on
//!   `wasm32-wasip1`).
...
impl Debouncer {
    ...
    /// Returns `true` if the caller should emit this evidence row.
    ///
    /// Semantics:
    /// - First time we see `(tool, class)`: allow, record.
    /// - Seen within window: deny, do NOT advance timestamp (so a
    ///   sustained attack still produces exactly one row per window).
    /// - Seen but window elapsed: allow, refresh timestamp + LRU
    ///   position.
    pub fn should_emit(
        &mut self,
        tool: &str,
